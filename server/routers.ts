...
export const appRouter = router({
  ...
  academy: router({
    ...
    notifications: router({
      list: protectedProcedure.input(z.object({ unreadOnly: z.boolean().optional() }).optional()).query(({ ctx, input }) => listNotifications(ctx.user.id, input?.unreadOnly ?? false)),
      ...
      updatePreferences: protectedProcedure.input(z.object({ emailEnabled: z.boolean(), courseUpdates: z.boolean(), assessmentUpdates: z.boolean(), paymentUpdates: z.boolean(), companyUpdates: z.boolean(), securityUpdates: z.boolean() })).mutation(({ ctx, input }) => updateNotificationPreferences(ctx.user.id, input)),
    }),
    admin: router({
